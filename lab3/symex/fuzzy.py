...
class concolic_str(str):
  ...
  def get_concrete_string(self):
    if isinstance(self,concolic_str):
      return self.__v
    else:
      self
...
def get_next_constraints(current_path_constraints,checked,i,actual_len):
  new_constraint = [c for c in current_path_constraints]
  new_constraint2= [c for c in current_path_constraints]
  '''
  new_constraint[len(new_constraint)-1]=sym_not(new_constraint[len(new_constraint)-1])
  new_constraint_conjunct = sym_and(*new_constraint)

  if new_constraint_conjunct not in checked:
    print "\n"*5+"checked"+"\n"*5
    return new_constraint
  '''
  new_constraint = [c for c in current_path_constraints]
  for x in range(i,actual_len):
    new_constraint[x]=sym_not(new_constraint[x])
    new_constraint_conjunct = sym_and(*new_constraint)
    if new_constraint_conjunct not in checked:
      return new_constraint
  
  if i>=0:
    return get_next_constraints(new_constraint2,checked,i-1,actual_len)

  return None



  #new_constraint[i]=sym_not(new_constraint[i])
  #temp=sym_and(*new_constraint)
  
  # if temp not in checked:
  #   return new_constraint
  # elif i>=0 and i<actual_len:
  #   for ptr in range(i,actual_len):

  #   return get_next_constraints(cur_path_constr,checked,i-1,actual_len)
  # return None
...
def concolic_test(testfunc, maxiter = 100, verbose = 0):
  ...
  while iter < maxiter and not inputs.empty():
    ...
    global concrete_values                #CONCRETE VALUES IS A DICT
    ...
    # for i in cur_path_constr:
    #   cpc=i
    #   cpconstraint=[sym_not(cpc)]
    #   for c in cur_path_constr:
    #     if c!=i:
    #       cpconstraint.append(c)

    #   cpconstraint=sym_and(*cpconstraint)

    #   if cpconstraint not in checked:
    #     checked.add(cpconstraint)
    #   else:
    #     continue

    #   (ok,model)=fork_and_check(cpconstraint)

    #   if ok==z3.sat:
    #     inputs.add(model,cur_path_constr_callers[cur_path_constr.index(i)])

    for i in range(len(cur_path_constr)):

      cpc=[sym_not(cur_path_constr[i])]
      for j in range(i):
        cpc.append(cur_path_constr[j])

      cpconstraint=sym_and(*cpc)
    ...
    # for i in range(len(cur_path_constr)):
    #   ctr=[]
    #   ctr.append(sym_not(cur_path_constr[i]))

    #   for j in range(len(cur_path_constr)):
    #     if j!=i:
    #       ctr.append(cur_path_constr[j])

    #   constraint_all=sym_and(*ctr)

    #   (ok,model)=fork_and_check(constraint_all)
    ...
    # new_constraint = get_next_constraints(cur_path_constr,checked,len(cur_path_constr)-1,len(cur_path_constr))
    
    # if new_constraint==None:
    #   continue
    
    # constraint_all = sym_and(*new_constraint)
    # if constraint_all not in checked:
    #   checked.add(constraint_all)

    #   #print "\n\nNew Path Constraints"
    #   print constraint_all
    #   #print constraint_all
    #   #model returns the dict 
    #   (ok,model)=fork_and_check(constraint_all)

    #   if ok==z3.sat:
    #     for k in model.keys():
    #       concrete_valconcolic_testues[k]=model[k]

    #   #print concrete_values
    #   inputs.add(concrete_values,None)      #first argument is the new set of values and the second is the callers info that can be passed as None since it is purely used for debuggin purpse
